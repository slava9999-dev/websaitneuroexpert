...
- `api/index.py` tried to import non-existent `setup_routes` function
- Routes were not being mounted to the main FastAPI app

**Solution:**
- Создан единый `backend/routes.py`, который экспортирует `setup_routes`
- `api/index.py` импортирует маршруты из пакета `backend`
- Маршруты монтируются с корректным доступом к базе данных
- Добавлен fallback-клиент MongoDB, если `app.state.db` недоступен
...
**Files Changed:**
- `api/index.py` (health endpoint already present, verified)
...
**Files Changed:**
- `api/index.py` (CORS already configured, verified)
