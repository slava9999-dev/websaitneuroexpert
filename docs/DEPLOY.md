...
- **`/api/`** — Vercel serverless entrypoints (`index.py`, `chat.js`)
- **`/backend/`** — основное FastAPI приложение, SmartContext, конфигурации
- **`/frontend/`** — React 18 (CRA) с Tailwind, UI-компонентами и видеофоном
- **`docs/`** — документация по деплою, DevOps и health-check
