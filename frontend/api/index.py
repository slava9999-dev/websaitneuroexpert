...
SENTRY_DSN = os.getenv("SENTRY_DSN", OPTIONAL_ENV["SENTRY_DSN"])
REDIS_URL = os.getenv("REDIS_URL", OPTIONAL_ENV["REDIS_URL"])
SQS_QUEUE_URL = os.getenv("SQS_QUEUE_URL", OPTIONAL_ENV["SQS_QUEUE_URL"])
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # ============================================================================
        # Время жизни документов в секундах (по умолчанию 30 дней)
        chat_ttl = int(os.getenv("CHAT_TTL_SECONDS", 2592000))
        await app.state.db.chat_messages.create_index(
            "timestamp", expireAfterSeconds=chat_ttl
        )
        logger.info(
            f"✅ Set TTL index on chat_messages (expireAfterSeconds={chat_ttl})"
        )
        contact_ttl = int(os.getenv("CONTACT_TTL_SECONDS", 2592000))
        await app.state.db.contact_forms.create_index(
            "timestamp", expireAfterSeconds=contact_ttl
        )
        logger.info(
            f"✅ Set TTL index on contact_forms (expireAfterSeconds={contact_ttl})"
        )
    ...
    logger.info("✅ Redis connected successfully", extra={"redis_url": REDIS_URL.split('@')[-1]})
...
# ============================================================================
# CORS Configuration (dynamic via ALLOWED_ORIGINS env)
# ============================================================================
# Разбор списка разрешённых origins из env ALLOWED_ORIGINS (comma-separated) или из CLIENT_ORIGIN
orig_env = os.getenv("ALLOWED_ORIGINS", CLIENT_ORIGIN)
origins = [o.strip() for o in orig_env.split(',') if o.strip()]
# Делим на точные совпадения и шаблоны
allowed_exact = []
regex_patterns = []
for o in origins:
    if '*' in o:
...
# Строим общий regex для шаблонов
