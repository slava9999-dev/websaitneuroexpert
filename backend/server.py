...
if not GOOGLE_API_KEY:
    raise RuntimeError("FATAL: GOOGLE_API_KEY must be set in environment variables")
...
Будьте живым, полезным экспертом, который искренне хочет помочь решить задачу клиента!"""
        ...
        # Prepare messages for Google Gemini chat
        messages = [
            {"author": "system", "content": system_prompt},
            {"author": "user", "content": chat_request.message}
        ]
        # Call Google Gemini model
        genai_response = genai.chat.completions.create(
            model="models/chat-bison-001",
            messages=messages
        )
        response = genai_response.candidates[0].content
        ...
        message_record = {
            ...
            "model": "google-gemini-chat-bison-001",
